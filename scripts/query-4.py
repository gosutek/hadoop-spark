...
stations_path = (
    "hdfs://advdb-master:54310/user/master/other_data/LAPD_Police_Stations.csv"
)
...
# join_1.explain()
join_1 = join_1.withColumn(
    "pd_distance",
    udf(lambda lat1, lon1, lat2, lon2: get_distance(lat1, lon1, lat2, lon2))(
        join_1.LAT, join_1.LON, join_1.Y, join_1.X
    ),
).cache()
query4_1_df = (
    join_1.groupBy(year("Date Rptd").alias("year"))
    .agg(
        round(avg("pd_distance"), 3).alias("average_distance"),
        count("DR_NO").alias("#"),
    )
...
# Query4-2
query4_2_df = (
    join_1.groupBy(col("DIVISION").alias("division"))
    .agg(
        round(avg("pd_distance"), 3).alias("average_distance"),
        count("DR_NO").alias("#"),
    )
...
query4_2_df.show(5)
# Query4-3
min_df = query4_df.withColumn(
    "result",
    udf(
        lambda lat1, lon1: min_distance(lat1, lon1),
        StructType(
            [StructField("dst", DoubleType()), StructField("div", StringType())]
        ),
...
query4_3_df = (
    min_df.groupBy(year("Date Rptd").alias("year"))
    .agg(
        round(avg("pd_distance"), 3).alias("average_distance"),
        count("DR_NO").alias("#"),
    )
...
# Query4-4
query4_4_df = (
    min_df.groupBy(col("DIVISION").alias("division"))
    .agg(
        round(avg("pd_distance"), 3).alias("average_distance"),
        count("DR_NO").alias("#"),
    )
